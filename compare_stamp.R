...
diff$p.adjust <- p.adjust(diff$p.value, opts$fdr)
diff <- diff %>% filter(p.value < opts$pvalue)
}
...
### 绘图数据构建
### 左侧条形图
##print(head(data1,n=4))
##print(head(diff$var,n=3))
abun.bar <- data1[,c(diff$var,"Group")] %>%
    gather(variable,value,-Group) %>%
    group_by(variable,Group) %>%
    summarise(Mean=mean(value))
##print(head(data1,n=3))
##print(head(abun.bar,n=2))
### 右侧散点图
##diff.mean <- diff[,c("var","estimate","conf.low","conf.high","p.value")]
#diff.mean <- diff[,c("var","statistic","p.value","p.adjust","method","alternative")]
...
filename2=paste0(opts$output, "_", basename(opts$input),".abun")
write.table(diff, file=filename, append=F, quote=F, sep='\t', row.names=F, col.names=T)
write.table(abun.bar, file=filename2, append=F, quote=F, sep='\t', row.names=F, col.names=T)

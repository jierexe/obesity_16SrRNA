...
main_theme = theme(panel.background=element_blank(), panel.grid=element_blank(),
	...
	axis.ticks=element_line(color="black"), axis.text=element_text(color="black", size=8),
	legend.position="right", legend.background=element_blank(), legend.key=element_blank(), legend.text= element_text(size=8),
	text=element_text(family="sans", size=8))

# 实验差异比较结果
x = read.table("/data/project/LBB-16S/feipang/diff/result/compare/O-N.txt", header=T, row.names= 1, sep="\t", stringsAsFactors = F)
# 只提取前14列
x = x[,1:7]
x = na.omit(x)

# 添加物种注释
taxonomy = read.table("/data/project/LBB-16S/feipang/diff/result/taxonomy.txt", sep = "\t", row.names=1, header=T, stringsAsFactors = F)
...
# 读取高丰度门，用于着色
per= read.delim("/data/project/LBB-16S/feipang/diff/result/otu_table.Family.relative.xls", sep = "\t", row.names=1, header=T)
mean = rowMeans(per)
per = as.data.frame(mean[order(mean, decreasing = T)])
top_tax=head(rownames(per), n=12)
...
p = ggplot(x, aes(x=num, y=neglogp, color=tax, size=log2CPM, shape=level)) +
  ...
  labs(x="Family of Features", y="-log10(P)") +
  labs(title=paste(gsub(".txt", "", basename("/data/project/LBB-16S/feipang/diff/result/compare/O-N.txt")), sep=" ")) +
  main_theme +
...
p
ggsave(file=paste("/data/project/LBB-16S/feipang/diff/result/compare/O-N.manhattan.f.legend.pdf", sep=""), p, width = 140, height = 102, useDingbats=F, units = "mm")
# ggsave(file=paste("/data/project/LBB-16S/feipang/diff/result/compare/O-N.txt_manhattan.png", sep=""), p, width = 140, height = 102)
